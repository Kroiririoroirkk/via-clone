...
_via_file_annotator.prototype._file_html_element_compute_scale = function() {
  ...
  case _VIA_ZOOM_MODE.FITWIDTH:
    ...
    break;
  default:
    zoom_scale_value = _VIA_ZOOM_SCALE_VALUE_LIST[this.va.zoom_scale_value_index];
  ...
  if ( this.d.store.file[this.fid].type === _VIA_FILE_TYPE.VIDEO && this.va.zoom_mode === _VIA_ZOOM_MODE.SCALE ) {
    this.file_html_element_size_css += 'transform:scale(' + zoom_scale_value + ');transform-origin:left top;';
  }
  ...
  case 'center':
    this.left_pad = Math.floor( (maxw - this.cwidth*zoom_scale_value) / 2 );
    if ( this.left_pad > 0 ) {
      this.file_html_element_size_css += 'left:' + this.left_pad + 'px;';
    }
    break;
  case 'right':
    this.left_pad = maxw - this.cwidth*zoom_scale_value;
    if ( this.left_pad > 0 ) {
      this.file_html_element_size_css += 'left:' + this.left_pad + 'px;';
    }
    break;
...
_via_file_annotator.prototype._smetadata_set_position = function() {
  ...
  if ( this.left_pad > 0 ) {
    var x = this.left_pad + this.creg[mid][1]*zoom_scale_value;
  } else {
    var x = this.creg[mid][1]*zoom_scale_value;
  }  
  var y = this.conf.REGION_SMETADATA_MARGIN + this.creg[mid][2]*zoom_scale_value;
  var shape_id = this.creg[mid][0];
  switch(shape_id) {
  case _VIA_RSHAPE.CIRCLE:
    y = y + this.creg[mid][3];
